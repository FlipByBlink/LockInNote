...
enum 💾UserDefaults {
    private static let ⓤserDefaults = UserDefaults(suiteName: "group.net.aaaakkkkssssttttnnnn.LockInNote")
    enum 🄺ey: String {
        case Rectangular, Circular, Inline
    }
    static func load<T: Codable>(_ ⓚey: Self.🄺ey) -> T? {
        guard let ⓓata = Self.ⓤserDefaults?.data(forKey: ⓚey.rawValue) else { return nil }
        do {
